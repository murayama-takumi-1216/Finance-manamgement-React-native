...
const AccountCalendarScreen = ({ route, navigation }) => {
  ...
  const currentAccount = accounts.find(a => a.id === accountId || a.id === parseInt(accountId));
  ...
  }, [fetchEvents, fetchReminders, accountId]);
  ...
  const handleSaveReminder = async () => {
    ...
    try {
      ...
      const result = await createReminder(accountId, payload);
  ...
  const handleDeleteReminder = async () => {
    ...
    setSaving(true);
    try {
      const result = await deleteReminder(accountId, selectedReminder.id);
      if (result.success) {
        Toast.show({
          type: 'success',
  ...
  const getEventDate = (event) => {
    return event.fecha || event.fechaHoraInicio || event.fecha_hora_inicio;
  };
                      ...
                      <View style={styles.eventInfo}>
                        ...
                        {event.monto && (
                          <Text style={styles.eventAmount}>
                            {formatCurrency(event.monto, currentAccount?.moneda)}
                          </Text>
                        )}
                        {event.recurrencia && (
